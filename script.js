function load(){
    document.getElementById('pallet').innerHTML = "<div style='color:gold;'>Select a shape [ &#9651; or &#9645; or &#9633; ] to start <br>اختر شكلاً [ &#9651; أو &#9633; أو &#9645; ] للبدء</div>";
  }
  function set_history(history) {
    let shape_icon = {square:' &#9633;',triangle:'&#9651;',rectangle:'&#9645;'};
    var shape = shape_icon[document.getElementById('shape').value];
    var pallet = document.getElementById('pallet').innerHTML;
    history[history.length] = "<div class='element'>"+(history.length+1)+"- "+shape+" at "+ new Date().toLocaleTimeString()+"<table id='_"+history.length+"'>"+document.getElementById('table_body').innerHTML+"</table></div>";
  ...
  function check(id) {
    document.getElementById('rectangle_shape').classList.remove('checked');
  ...
  function fill_data(slid_r,slid_t) {
    document.getElementById('slid_r_r').value= slid_r;
    document.getElementById('slid_r_b').value= slid_t;
    document.getElementById('slid_r_l').value= slid_r;
    document.getElementById('slid_r_t').value= slid_t;
    }
  ...
  function select_shape(value) {
    ...
    const rectangles = document.getElementsByClassName('rq');
    ...
    if(value == 'rectangle'){
        ...
        for (let i = 0; i < triangles.length; i++)
        triangles[i].style.display = 'none';
        change(document.getElementById('slid_r_r').id);
        change(document.getElementById('slid_r_t').id);
      }
  }
  function change(id){
  ...
  if(id[5] == 'r'){
    ...
    var height = document.getElementById('slid_r_r').value;
    var width = document.getElementById('slid_r_t').value;
  if(id == 'slid_r_r'){
    document.getElementById('slid_r_l').value = height;
  }
  if(id == 'slid_r_t'){
    document.getElementById('slid_r_b').value = width;
  }
  
  document.getElementById('pallet').innerHTML = "<style>.rectangle {margin: 0 auto;position: relative;width: 0;height: 0;border-right: "+height+"cm solid cadetblue;border-bottom: "+width+"cm solid cadetblue;border-left: "+height+"cm solid cadetblue;border-top: "+width+"cm solid cadetblue;}</style><div class='rectangle'></div>";

  document.getElementById('info').innerHTML = "<label>Left side ="+height+" cm</label><br><label>Right side ="+height+" cm</label><br><label>Bottom side ="+width+" cm</label><br><label>Top side ="+width+" cm</label><br>";
